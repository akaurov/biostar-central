...
class UserEditForm(forms.Form):
    name = forms.CharField(help_text=_("The name displayed on the site (required)"))

    email = forms.EmailField(help_text=_("Your email, it will not be visible to other users (required)"))

    location = forms.CharField(required=False,
                               help_text=_("Country/City/Institution (recommended)"))

    website = forms.URLField(required=False, max_length=100,
                             help_text=_("The URL to your website (optional)"))

    twitter_id = forms.CharField(required=False, max_length=15,
                                 help_text=_("Your twitter id (optional)"))

    scholar = forms.CharField(required=False, max_length=15,
                              help_text=_("Your Google Scholar ID (optional)"))

    my_tags = forms.CharField(max_length=100, required=False,
                              help_text=_("Post with tags listed here will show up in the My Tags tab. Use a comma to separate tags. Add a <code>!</code> to remove a tag. Example: <code>galaxy, bed, solid!</code> (optional)"))

    watched_tags = forms.CharField(max_length=100, required=False,
                                   help_text=_("Get email when a post matching the tag is posted. Example: <code>minia, bedops, breakdancer, music</code>."))

    digest_prefs = forms.ChoiceField(required=True, choices=Profile.DIGEST_CHOICES, label=_("Email Digest"),
                                     help_text=_("(This feature is not working yet!). Sets the frequence of digest emails. A digest email is a summary of events on the site."))

    message_prefs = forms.ChoiceField(required=True, choices=const.MESSAGING_TYPE_CHOICES, label=_("Notifications"),
                                      help_text=_("Default mode  sends you an email if you receive anwers to questions that you've posted."))

    info = forms.CharField(widget=forms.Textarea, required=False, label=_("Add some information about yourself"),
                           help_text=_("A brief description about yourself (recommended)"))
...
class EditUser(LoginRequiredMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not target.has_ownership:
            messages.error(request, _("Only owners may edit their profiles"))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if User.objects.filter(email=f['email']).exclude(pk=request.user.id):
                ...
                messages.error(request, _("The email that you've entered is already registered to another user!"))
                return render(request, self.template_name, {'form': form})
...
class DigestForm(forms.Form):
    digest_prefs = forms.ChoiceField(required=True, choices=Profile.DIGEST_CHOICES, label=_("Emails"),
                                     help_text=_("Sets the frequence of digest emails"))
...
def unsubscribe(request, uuid):
    ...
    if not user:
        ...
        messages.success(request, _('User unsubscribed.'))
...
class DigestManager(LoginRequiredMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            user.profile.save()
            messages.success(request, _('Email frequency has been set to %s') % user.profile.get_digest_prefs_display())
